...
public class UserSettingsRepository : IUserSettingsRepository
{
    ...
    public async Task<UserPomodoroSettingsEntity?> GetUserPomodoroSettingsAsync(string userId)
    {
        ...
        var setting = await collection.Find(filter).FirstOrDefaultAsync();
        return setting;
    }

    public async Task SavePomodoroSettings(UserPomodoroSettingsEntity entity)
    {
        var client = new MongoClient(_mongoSettiings.ConnectionString);
        var collection = client.GetDatabase(_mongoSettiings.Database).GetCollection<UserPomodoroSettingsEntity>(_mongoSettiings.Collections?.UserSettingsCollection);
        var filter = Builders<UserPomodoroSettingsEntity>.Filter.Eq(x => x.UserID, entity.UserID);

        await collection.UpdateOneAsync(filter, new UpdateDefinitionBuilder<UserPomodoroSettingsEntity>().Set(x => x.MinutesPerPomodoro, entity.MinutesPerPomodoro)
            .Set(x => x.MinutesPerShortBreak, entity.MinutesPerShortBreak)
            .Set(x => x.MinutesPerLongBreak, entity.MinutesPerLongBreak)
            .Set(x => x.PomodorosBeforeLongBreak, entity.PomodorosBeforeLongBreak), new UpdateOptions { IsUpsert = true });
    }
}
